...
#include <rt_hw_accel_msgs/msg/point.h>
#include <rt_hw_accel_msgs/msg/point_array.h>
...
void corners_subscription_callback(const void * msgin)
{
	const rt_hw_accel_msgs__msg__PointArray * msg = (const rt_hw_accel_msgs__msg__PointArray *)msgin;
	printf("Corners subscription callback: %d\n", ++corners_cb_count);
}
...
void main(void)
{
	...
	RCCHECK(rclc_subscription_init_best_effort(&corners_subscriber, &node, ROSIDL_GET_MSG_TYPE_SUPPORT(rt_hw_accel_msgs, msg, PointArray), "/microROS/corners"));
	...
	RCCHECK(rclc_executor_add_subscription(&executor, &corners_subscriber, &corners, &corners_subscription_callback, ON_NEW_DATA));
